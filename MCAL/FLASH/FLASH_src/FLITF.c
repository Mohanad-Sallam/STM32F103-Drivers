...
#define MOST_SIGNIFICANT_BYTE 0xff00
...
void Flash_ProgramWrite(void * StartAddress,void * DataAddress,uint_32t DataLength){
	...
	while((DataLength - HALF_WORD_LEN*Iterator) >= HALF_WORD_LEN ){
		( ( uint_16t *) StartAddress )[Iterator]  = ( ( uint_16t *) DataAddress )[Iterator];
		Iterator ++;
	}
	if((DataLength - HALF_WORD_LEN*Iterator)){
		temp =(( (uint_8t *) DataAddress )[HALF_WORD_LEN*Iterator])  | MOST_SIGNIFICANT_BYTE;
