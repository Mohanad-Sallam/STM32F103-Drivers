...
extern DMAConfig_t DMA_ChannelsConfig[MAX_NUMBER_OF_CHANNELS];
...
uint_8t DDMA_Init(void)
{
	...
	for(Counter=0;Counter<MAX_NUMBER_OF_CHANNELS;Counter++)
	{
		...
		if(( DMA_ChannelsConfig[Counter].ChannelNumber ) < 7 )
		{

			DMA->Channel[DMA_Configure[Counter].ChannelNumber].CCR=
					DMA_ChannelsConfig[Counter].Mem2Mem |
					DMA_ChannelsConfig[Counter].PL      |
					DMA_ChannelsConfig[Counter].MSIZE   |
					DMA_ChannelsConfig[Counter].PSIZE   |
					DMA_ChannelsConfig[Counter].MINC    |
					DMA_ChannelsConfig[Counter].PINC    |
					DMA_ChannelsConfig[Counter].CIRC    |
					DMA_ChannelsConfig[Counter].DIR     |
					DMA_ChannelsConfig[Counter].TEIE    |
					DMA_ChannelsConfig[Counter].HTIE    |
					DMA_ChannelsConfig[Counter].TCIE    ;
					ErrorStatus|=DNVIC_ClearPendingIRQ(DMA_ChannelsConfig[Counter].ChannelNumber+DMA_2_NVIC);
					ErrorStatus|=DNVIC_EnableIRQ(DMA_ChannelsConfig[Counter].ChannelNumber+DMA_2_NVIC);
		}
		else
		{
			ErrorStatus|=NOK;
...
uint_8t DDMA_Start(uint_32t MemoryAddress,uint_32t PeripheralAddress,uint_32t Counter,uint_8t ChannelNumber)
{
	uint_8t ErrorStatus=OK;
...
uint_8t DDMA_SetNotifyCbf(Notify_t *Notification, uint_8t Channel_Number)
{
	uint_8t ErrorStatus=OK;
