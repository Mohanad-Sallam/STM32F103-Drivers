...
uint_8t HUART_Init(void)
{
	...
	/*configure DGPIO Pins*/
	DGPIO_t DGPIO;
	DGPIO.Mode=MODE_PIN10_IP_PUR|MODE_PIN9_AF_PP;
	DGPIO.Speed=SPEED_PIN10_INPUT|SPEED_PIN9_2MHZ;
	DGPIO.Port=PORT_A;
	DGPIO.Pin=PIN_9|PIN_10;
	Local_Error=DGPIO_Config(&DGPIO);
	...
	Local_Error|=DUART_Init(BaudRate_Mantissa,BaudRate_Fraction,PARITY_BIT,DATA_BITS,STOP_BITS);
	return Local_Error;
}
...
/*motawa7esh uart */
	...
	localError= DUART_Send(Buffer,Length);
...
uint_8t HUART_Receive(uint_8t *Buffer, uint_16t Length)
{
	...
	localError= DUART_Receive(Buffer,Length);
...
uint_8t HUART_Config(uint_32t BaudRate,uint_32t ParityBits,uint_32t DataSize,uint_32t StopBits)
{
	...
	if((ParityBits==PARITY_DISABLED || ParityBits==PARITY_EVEN || ParityBits==PARITY_ODD) && (DataSize==DATA_8_BITS || DataSize==DATA_9_BITS) && (StopBits==ONE_STOP_BIT || StopBits==TWO_STOP_BIT))
	{
		...
		Local_Error=DUART_Config(BaudRate_Mantissa,BaudRate_Fraction,ParityBits,DataSize,StopBits);
	}
	else
	{
		Local_Error=NOT_OK;
...
uint_8t HUART_SetTxCbf(TxCbf_t TxCbf)
{
	...
	if(TxCbf)
	{
		LocalError = DUART_SetTxCbf(TxCbf);
	}
	else
	{
		LocalError=NOT_OK;
...
uint_8t HUART_SetRxCbf(RxCbf_t RxCbf)
{
	...
	if(RxCbf)
	{
		LocalError = DUART_SetRxCbf(RxCbf);
	}
	else
	{
		LocalError=NOT_OK;
